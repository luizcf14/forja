...
class Database
{
    ...
    private function initDb()
    {
        ...
        // Create User Requests Table (with original_message)
        ...
        // Add original_message column if not exists
        try {
    ...
    public function insertUserRequest($userIdentifier, $requestText, $importance = 'normal', $originalMessage = '')
    {
        $stmt = $this->pdo->prepare("INSERT INTO user_requests (user_identifier, request_text, importance, original_message, created_at) VALUES (?, ?, ?, ?, ?)");
        return $stmt->execute([$userIdentifier, $requestText, $importance, $originalMessage, date('Y-m-d H:i:s')]);
    }
